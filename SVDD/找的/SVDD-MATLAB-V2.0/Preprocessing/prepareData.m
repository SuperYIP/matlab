function [trainData, trainLabel, testData, testLabel] = prepareData(dataName)
    ...
    switch dataName
        case 'banana'
            tmp = load ('E:\Code\matlab\SVDD\找的\SVDD-MATLAB-V2.0\data\banana.mat');
            data = tmp.banana(:, 1:2);
            index_p = (tmp.banana(:, 3) == 0);

        case 'wine'
            tmp = load ('E:\Code\matlab\SVDD\找的\SVDD-MATLAB-V2.0\data\wine.mat');
            data = tmp.wine(:, 2:14);
            index_p = (tmp.wine(:, 1) == 1);

        case 'heart_scale'
            tmp = load ('E:\Code\matlab\SVDD\找的\SVDD-MATLAB-V2.0\data\heart_scale.mat');
            data = tmp.heart_scale_inst;
            label = tmp.heart_scale_label;
            index_p = (label(:,1) == 1);

        case 'industial'
            tmp = load ('E:\Code\matlab\SVDD\找的\SVDD-MATLAB-V2.0\data\industial.mat');
            data = tmp.data;
            label = tmp.label;
            index_p = (label(:,1) == 1);

        case 'iris'
            tmp = load ('E:\Code\matlab\SVDD\找的\SVDD-MATLAB-V2.0\data\iris.mat');
            data = tmp.data;
            label = tmp.label;
            index_p = (label(:,1) == 1);
    end
